...
```{r}
dfmpg <- data.frame(Manufacturer = data$manufacturer, Model = data$model)
...
geom_bar(stat = "identity", fill = "green")+ theme(plot.title = element_text(hjust = 0.5),
...
```{r}
library(ggplot2)
ggplot(data, aes(model, manufacturer)) + geom_point()+ theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(angle = 90, hjust = 1))
...
data %>%
 group_by(model) %>%
 summarize(count = n()) -> grouped_data
...
```{r}
mpg_observation_top20 <- head(grouped_data, 20)
...
```{r}
library(ggplot2)
mpg_top20 <- ggplot(grouped_data, aes(x = model, y = count, fill = model)) +
  geom_bar(stat = "identity") +
  labs(title = "Number of Cars per Model",
       x = "Model" ,
       y = "Number of cars") +
...
relationship_cyl_displ <- ggplot(data, aes(x = cyl, 
                                               y = displ, 
                                               color = displ)) +
  geom_point() +
...
```{r}
displ_hwy_relationship <- ggplot(data, aes(x = displ, y = hwy, color = cty)) +
  geom_point() +
...
open_alexa_data <- read_excel("/cloud/project/Worksheet 4/Rworksheet_4c/alexa_file_.xlsx")
